...
# 创建一个 CSV 文件并添加表头
output_file="rsem_isoforms_tpm.csv"
header="gene_id"

# 提取第一个文件的 gene_id 和 TPM 列
first_file=$(ls *.isoforms.results | head -n 1)
awk 'NR > 1 {print $1 "," $6}' "$first_file" > "$output_file"

# 获取第一个文件的列名（作为列名）
first_file_name=$(basename "$first_file" .isoforms.results)
header="$header,$first_file_name"

# 提取后续文件的 TPM 列并合并
for file in *.isoforms.results; do
    if [ "$file" != "$first_file" ]; then
        # 获取文件名作为列名
        file_name=$(basename "$file" .isoforms.results)
...
echo "合并完成，结果保存在 rsem_isoforms_tpm.csv"
